#include "rubi_control_core/rubi_w_policy_adapter.hpp"

#include <algorithm>
#include <array>
#include <cmath>
#include <limits>
#include <stdexcept>
#include <string>

namespace rubi_control_core {
namespace {

constexpr double kPi = 3.14159265358979323846;
constexpr std::array<std::size_t, 6> kLegIndices{0, 1, 2, 4, 5, 6};
constexpr std::array<std::size_t, 2> kWheelIndices{3, 7};

template <typename Container>
bool all_finite(const Container& values) {
  return std::all_of(values.begin(), values.end(),
                     [](const auto value) { return std::isfinite(value); });
}

std::array<double, 3> projected_gravity(
    const std::array<double, 4>& quaternion_xyzw) {
  double x = quaternion_xyzw[0];
  double y = quaternion_xyzw[1];
  double z = quaternion_xyzw[2];
  double w = quaternion_xyzw[3];
  const double norm = std::sqrt(x * x + y * y + z * z + w * w);
  if (!std::isfinite(norm) || norm <= std::numeric_limits<double>::epsilon()) {
    throw std::runtime_error("invalid RUBI_W base orientation quaternion");
  }
  x /= norm;
  y /= norm;
  z /= norm;
  w /= norm;
  return {2.0 * (y * w - x * z),
          -2.0 * (y * z + x * w),
          -(1.0 - 2.0 * (x * x + y * y))};
}

}  // namespace

const std::array<std::string, kRubiWJointCount>& rubi_w_controller_order() {
  // These are source enum symbols, not unverified simulator joint names.
  static const std::array<std::string, kRubiWJointCount> names{
      "LHR", "LHP", "LKN", "LWH", "RHR", "RHP", "RKN", "RWH"};
  return names;
}

void validate_rubi_w_controller_order(
    const std::vector<std::string>& controller_order) {
  const auto& expected = rubi_w_controller_order();
  if (controller_order.size() != expected.size() ||
      !std::equal(controller_order.begin(), controller_order.end(),
                  expected.begin())) {
    throw std::invalid_argument(
        "controller order does not match the eight-axis RUBI_W source contract");
  }
}

RubiWPolicyAdapter::RubiWPolicyAdapter(
    std::shared_ptr<RubiWPolicyInterface> policy, RubiWContract contract)
    : policy_(std::move(policy)), contract_(std::move(contract)) {
  if (!policy_) {
    throw std::invalid_argument("RUBI_W policy runner must not be null");
  }
  if (contract_.physics_dt <= 0.0 || contract_.inference_decimation == 0 ||
      contract_.inference_dt <= 0.0 || contract_.walk_ready_duration <= 0.0 ||
      contract_.gait_frequency <= 0.0) {
    throw std::invalid_argument("invalid RUBI_W controller timing contract");
  }
  reset();
}

void RubiWPolicyAdapter::reset() {
  mode_ = ControllerMode::kTorqueOff;
  emergency_latched_ = false;
  walk_ready_complete_ = false;
  physics_tick_count_ = 0;
  mode_tick_count_ = 0;
  inference_count_ = 0;
  held_command_.fill(0.0);
  gait_index_ = 0.0;
  observation_.fill(0.0F);
  history_.fill(0.0F);
  latent_.fill(0.0F);
  policy_input_.fill(0.0F);
  action_.fill(0.0F);
  output_ = RubiWControllerOutput{};
}

bool RubiWPolicyAdapter::state_is_finite(
    const RubiWRobotState& state) const {
  return std::isfinite(state.sim_time_sec) && all_finite(state.joint_position) &&
         all_finite(state.joint_velocity) && all_finite(state.measured_effort) &&
         all_finite(state.base_orientation_xyzw) &&
         all_finite(state.base_angular_velocity);
}

void RubiWPolicyAdapter::build_observation(const RubiWRobotState& state) {
  gait_index_ += contract_.inference_dt * contract_.gait_frequency;
  if (gait_index_ > 1.0) {
    gait_index_ -= 1.0;
  }

  for (std::size_t i = 0; i < 3; ++i) {
    observation_[i] = static_cast<float>(
        state.base_angular_velocity[i] * contract_.angular_velocity_scale);
  }
  const auto gravity = projected_gravity(state.base_orientation_xyzw);
  for (std::size_t i = 0; i < 3; ++i) {
    observation_[3 + i] = static_cast<float>(gravity[i]);
  }
  for (std::size_t i = 0; i < kLegIndices.size(); ++i) {
    const auto joint = kLegIndices[i];
    observation_[6 + i] = static_cast<float>(
        (state.joint_position[joint] - contract_.default_pose[joint]) *
        contract_.dof_position_scale);
  }
  for (std::size_t i = 0; i < kRubiWJointCount; ++i) {
    observation_[12 + i] = static_cast<float>(
        state.joint_velocity[i] * contract_.dof_velocity_scale);
    observation_[20 + i] = action_[i];
  }
  observation_[28] =
      static_cast<float>(std::sin(gait_index_ * 2.0 * kPi));
  observation_[29] =
      static_cast<float>(std::cos(gait_index_ * 2.0 * kPi));
  for (std::size_t i = 0; i < contract_.gait_parameters.size(); ++i) {
    observation_[30 + i] = static_cast<float>(contract_.gait_parameters[i]);
  }
  if (!all_finite(observation_)) {
    throw std::runtime_error("nonfinite RUBI_W observation");
  }
}

void RubiWPolicyAdapter::run_inference(const RubiWRobotState& state) {
  build_observation(state);
  std::move(history_.begin() + kRubiWObservationDim, history_.end(),
            history_.begin());
  std::copy(observation_.begin(), observation_.end(),
            history_.end() - kRubiWObservationDim);

  latent_ = policy_->encode(history_);
  if (!all_finite(latent_)) {
    throw std::runtime_error("nonfinite RUBI_W encoder output");
  }
  std::copy(latent_.begin(), latent_.end(), policy_input_.begin());
  std::copy(observation_.begin(), observation_.end(),
            policy_input_.begin() + kRubiWLatentDim);
  for (std::size_t i = 0; i < held_command_.size(); ++i) {
    const double normalized = std::clamp(
        held_command_[i] / contract_.command_input_limit[i], -1.0, 1.0);
    policy_input_[kRubiWLatentDim + kRubiWObservationDim + i] =
        static_cast<float>(normalized * contract_.command_cmd_vel_scale[i] *
                           contract_.command_policy_scale[i]);
  }
  if (!all_finite(policy_input_)) {
    throw std::runtime_error("nonfinite RUBI_W policy input");
  }
  action_ = policy_->act(policy_input_);
  if (!all_finite(action_)) {
    throw std::runtime_error("nonfinite RUBI_W policy action");
  }
  ++inference_count_;
}

void RubiWPolicyAdapter::enter_mode(ControllerMode mode) {
  if (emergency_latched_ && mode != ControllerMode::kEmergencyStop) {
    return;
  }
  if (mode == ControllerMode::kPolicyOn && !walk_ready_complete_) {
    return;
  }
  if (mode_ == mode) {
    return;
  }
  mode_ = mode;
  mode_tick_count_ = 0;
  if (mode == ControllerMode::kEmergencyStop) {
    emergency_latched_ = true;
  }
}

RubiWControllerOutput RubiWPolicyAdapter::safe_zero(
    const std::string& reason, bool faulted) {
  output_.action = action_;
  output_.target_position.fill(0.0);
  output_.target_velocity.fill(0.0);
  output_.effort.fill(0.0);
  output_.mode = mode_;
  output_.inference_ran = false;
  output_.safe_zero = true;
  output_.faulted = faulted;
  output_.fault_reason = reason;
  return output_;
}

RubiWControllerOutput RubiWPolicyAdapter::apply_pd(
    const RubiWRobotState& state,
    const RubiWJointArray& target_position,
    const RubiWJointArray& target_velocity) {
  output_.action = action_;
  output_.target_position = target_position;
  output_.target_velocity = target_velocity;
  output_.mode = mode_;
  output_.safe_zero = false;
  output_.faulted = false;
  output_.fault_reason.clear();
  for (std::size_t i = 0; i < kRubiWJointCount; ++i) {
    const double effort =
        contract_.kp[i] * (target_position[i] - state.joint_position[i]) +
        contract_.kd[i] * (target_velocity[i] - state.joint_velocity[i]);
    if (!std::isfinite(effort)) {
      emergency_latched_ = true;
      mode_ = ControllerMode::kEmergencyStop;
      return safe_zero("nonfinite RUBI_W PD effort", true);
    }
    output_.effort[i] = std::clamp(
        effort, -contract_.torque_limit[i], contract_.torque_limit[i]);
  }
  return output_;
}

RubiWControllerOutput RubiWPolicyAdapter::update(
    const RubiWRobotState& state, const RubiWUserCommand& command) {
  output_.inference_ran = false;
  if (command.reset && command.reset_sequence != last_reset_sequence_) {
    const auto reset_sequence = command.reset_sequence;
    const auto command_sequence = command.command_sequence;
    const auto mode_sequence = command.mode_sequence;
    reset();
    last_reset_sequence_ = reset_sequence;
    last_command_sequence_ = command_sequence;
    last_mode_sequence_ = mode_sequence;
    return safe_zero("reset", false);
  }
  if (command.command_sequence != last_command_sequence_) {
    held_command_ = {command.linear_x, command.linear_y, command.angular_z};
    last_command_sequence_ = command.command_sequence;
  }
  if (command.mode_sequence != last_mode_sequence_) {
    enter_mode(command.requested_mode);
    last_mode_sequence_ = command.mode_sequence;
  }
  if (command.emergency_stop ||
      command.requested_mode == ControllerMode::kEmergencyStop) {
    enter_mode(ControllerMode::kEmergencyStop);
  }

  if (!state_is_finite(state) || !all_finite(held_command_)) {
    emergency_latched_ = true;
    mode_ = ControllerMode::kEmergencyStop;
    ++physics_tick_count_;
    return safe_zero("nonfinite RUBI_W state or command", true);
  }
  if (emergency_latched_ || mode_ == ControllerMode::kEmergencyStop) {
    ++physics_tick_count_;
    return safe_zero("emergency stop latched", false);
  }
  if (mode_ == ControllerMode::kTorqueOff) {
    ++physics_tick_count_;
    return safe_zero("torque off", false);
  }

  RubiWJointArray target_position{};
  RubiWJointArray target_velocity{};
  if (mode_ == ControllerMode::kWalkReady) {
    const double elapsed =
        static_cast<double>(mode_tick_count_) * contract_.physics_dt;
    if (elapsed <= contract_.walk_ready_duration) {
      const double blend = 0.5 *
          (1.0 - std::cos(kPi * elapsed / contract_.walk_ready_duration));
      for (std::size_t i = 0; i < kRubiWJointCount; ++i) {
        target_position[i] = blend * contract_.walk_ready_pose[i];
      }
      ++mode_tick_count_;
    } else {
      target_position = contract_.walk_ready_pose;
      walk_ready_complete_ = true;
    }
  } else {
    if (physics_tick_count_ % contract_.inference_decimation == 0) {
      try {
        run_inference(state);
        output_.inference_ran = true;
      } catch (const std::exception& error) {
        emergency_latched_ = true;
        mode_ = ControllerMode::kEmergencyStop;
        ++physics_tick_count_;
        return safe_zero(error.what(), true);
      }
    }
    target_position = contract_.default_pose;
    for (const auto index : kLegIndices) {
      target_position[index] +=
          static_cast<double>(action_[index]) * contract_.action_scale[index];
    }
    for (const auto index : kWheelIndices) {
      target_velocity[index] =
          static_cast<double>(action_[index]) * contract_.action_scale[index];
    }
  }

  ++physics_tick_count_;
  auto result = apply_pd(state, target_position, target_velocity);
  result.inference_ran = output_.inference_ran;
  output_ = result;
  return output_;
}

}  // namespace rubi_control_core
