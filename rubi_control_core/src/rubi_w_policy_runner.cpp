...
std::array<float, OutputDim> infer(
    Ort::Session& session, const std::array<float, InputDim>& input) {
  ...
  if (outputs.size() != 1 || !outputs.front().IsTensor()) {
    throw std::runtime_error("RUBI_W ONNX inference did not return one tensor");
  }
  ...
  if (info.GetElementType() != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT ||
    ...
    throw std::runtime_error(
        "RUBI_W ONNX inference output contract changed at runtime");
  }
...
class RubiWPolicyRunner::Impl {
 public:
  Impl(const std::string& encoder_path, const std::string& policy_path)
      : environment(ORT_LOGGING_LEVEL_WARNING, "rubi_w_control_core"),
        encoder(nullptr),
        policy(nullptr) {
    ...
    validate_session(encoder, kRubiWEncoderInputDim, kRubiWLatentDim,
                     "RUBI_W encoder");
    validate_session(policy, kRubiWPolicyInputDim, kRubiWActionDim,
                     "RUBI_W policy");
  }
...
RubiWPolicyRunner::RubiWPolicyRunner(const std::string& encoder_path,
                                     const std::string& policy_path)
    : impl_(std::make_unique<Impl>(encoder_path, policy_path)) {}

RubiWPolicyRunner::~RubiWPolicyRunner() = default;
RubiWPolicyRunner::RubiWPolicyRunner(RubiWPolicyRunner&&) noexcept = default;
RubiWPolicyRunner& RubiWPolicyRunner::operator=(RubiWPolicyRunner&&) noexcept =
    default;

RubiWLatent RubiWPolicyRunner::encode(const RubiWHistory& history) {
  return infer<kRubiWEncoderInputDim, kRubiWLatentDim>(impl_->encoder, history);
}

RubiWAction RubiWPolicyRunner::act(const RubiWPolicyInput& input) {
  return infer<kRubiWPolicyInputDim, kRubiWActionDim>(impl_->policy, input);
}
